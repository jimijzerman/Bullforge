...
# Tabs
# =========================================================
tab_mobile, tab_market, tab_trade, tab_journal, tab_daily, tab_learning = st.tabs(["📱 Mobiele weergave", "📈 Markt", "🎯 Trade", "📝 Journal", "📒 Dagresultaten", "🧠 Leren"])
...
def render_live_market_and_trade_tabs():
    ...
    with tab_mobile:
        ...
        active_entry_zone, active_target_zone, active_invalidation_zone = _active_zones(selected_result)
        active_side = display_best_side or _active_side(selected_result) or "-"
        status = str(selected_result.get("status") or "-")
        price = selected_result.get("live_price") or selected_result.get("current_price")
        context_label = str(selected_result.get("context_label_main") or "-")
        setup_label = _setup_type_label(selected_result)
        reason_text = _one_line_reason(selected_result, trade_story)

        st.markdown(
            """
            <style>
                @media (max-width: 760px) {
                    .block-container { padding-left: 0.65rem !important; padding-right: 0.65rem !important; padding-top: 0.75rem !important; }
                    div[data-testid="stMetric"] { background: rgba(15,23,42,0.30); border: 1px solid rgba(148,163,184,0.18); border-radius: 14px; padding: 10px; }
                    div[data-testid="stHorizontalBlock"] { gap: 0.45rem !important; }
                    .stTabs [data-baseweb="tab-list"] { overflow-x: auto; white-space: nowrap; }
                    .bf-mobile-card { padding: 14px !important; border-radius: 18px !important; }
                    .bf-mobile-grid { grid-template-columns: 1fr !important; }
                }
                .bf-mobile-card {
                    ...
                    background: linear-gradient(180deg, rgba(17,24,39,0.74), rgba(15,23,42,0.58));
                    box-shadow: 0 10px 28px rgba(0,0,0,0.18);
                }
                .bf-mobile-title { font-size: 1.05rem; font-weight: 900; color: #F8FAFC; margin-bottom: 5px; }
                .bf-mobile-sub { font-size: 0.86rem; color: #AAB4C3; line-height: 1.35; }
                .bf-mobile-big { font-size: 1.65rem; font-weight: 950; color: #F8FAFC; margin: 6px 0; }
                .bf-mobile-pill { display:inline-block; padding: 4px 9px; border-radius: 999px; font-size: 0.78rem; font-weight: 850; margin-right: 5px; margin-top: 6px; background: rgba(59,130,246,0.16); color: #BFDBFE; border: 1px solid rgba(96,165,250,0.22); }
                .bf-mobile-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 10px; margin-top: 10px; }
                .bf-mobile-mini { border: 1px solid rgba(148,163,184,0.20); border-radius: 16px; padding: 12px; background: rgba(2,6,23,0.22); }
                .bf-mobile-label { font-size: 0.76rem; color: #94A3B8; font-weight: 800; margin-bottom: 4px; }
                .bf-mobile-value { font-size: 1.02rem; color: #F8FAFC; font-weight: 950; word-break: break-word; }
                .bf-mobile-wait { color: #FACC15; }
                .bf-mobile-good { color: #4ADE80; }
                .bf-mobile-bad { color: #F87171; }
            </style>
            """,
            unsafe_allow_html=True,
        )

        st.subheader("📱 Mobiele weergave")
        mobile_coin = st.selectbox(
            "Coin",
            list(COINS.keys()),
            index=list(COINS.keys()).index(st.session_state.selected_coin) if st.session_state.selected_coin in COINS else 0,
            key="mobile_coin_select",
        )
        if mobile_coin != st.session_state.selected_coin:
            st.session_state.selected_coin = mobile_coin
            st.rerun()

        headline_class = "bf-mobile-good" if trade_story.get("status_kind") == "success" else ("bf-mobile-wait" if trade_story.get("status_kind") == "warning" else "")
        st.markdown(
            f"""
            <div class="bf-mobile-card">
                <div class="bf-mobile-title">{selected_result['coin']} • {timeframe_label}</div>
                <div class="bf-mobile-big">{fmt_price_eur(float(price)) if price is not None else '-'}</div>
                <div class="bf-mobile-sub">
                    <span class="bf-mobile-pill">{context_label}</span>
                    <span class="bf-mobile-pill">{active_side}</span>
                    <span class="bf-mobile-pill">{setup_label}</span>
                </div>
                <div class="bf-mobile-title {headline_class}" style="margin-top:12px;">{trade_story.get('headline', 'Advies: Wachten')}</div>
                <div class="bf-mobile-sub">{reason_text}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )

        if display_best_metrics is not None and active_side in {"LONG", "SHORT"}:
            entry_text = fmt_price_eur(float(display_best_metrics["entry"]))
            stop_text = fmt_price_eur(float(display_best_metrics["stop"]))
            target_text = fmt_price_eur(float(display_best_metrics["target"]))
            rr_text = f"1 : {float(display_best_metrics['rr']):.2f}"
        else:
            entry_text = fmt_zone(active_entry_zone)
            stop_text = fmt_zone(active_invalidation_zone)
            target_text = fmt_zone(active_target_zone)
            rr_text = "-"

        st.markdown(
            f"""
            <div class="bf-mobile-card">
                <div class="bf-mobile-title">Tradeplan</div>
                <div class="bf-mobile-grid">
                    <div class="bf-mobile-mini"><div class="bf-mobile-label">ENTRY</div><div class="bf-mobile-value bf-mobile-good">{entry_text}</div></div>
                    <div class="bf-mobile-mini"><div class="bf-mobile-label">STOPLOSS</div><div class="bf-mobile-value bf-mobile-bad">{stop_text}</div></div>
                    <div class="bf-mobile-mini"><div class="bf-mobile-label">TAKE PROFIT</div><div class="bf-mobile-value">{target_text}</div></div>
                </div>
                <div class="bf-mobile-grid">
                    <div class="bf-mobile-mini"><div class="bf-mobile-label">RR</div><div class="bf-mobile-value">{rr_text}</div></div>
                    <div class="bf-mobile-mini"><div class="bf-mobile-label">STATUS</div><div class="bf-mobile-value">{_status_icon(status)} {status}</div></div>
                    <div class="bf-mobile-mini"><div class="bf-mobile-label">SCORE</div><div class="bf-mobile-value">{float(selected_result.get('score', 0) or 0):.0f}/100</div></div>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        if display_best_metrics is not None and active_side in {"LONG", "SHORT"}:
            if st.button("📝 Log deze trade", key=f"mobile_log_best_{selected_result['coin']}", use_container_width=True):
                entry = build_journal_entry(selected_result, active_side, "best", display_best_metrics)
                append_trade_journal(entry)
                st.success("Trade gelogd in journal.")
        else:
            st.button("Geen logbare trade", disabled=True, key="mobile_no_trade_button", use_container_width=True)

        render_range_position_bar(selected_result, side=active_side if active_side in {"LONG", "SHORT"} else None)
        render_volume_panel(selected_result, compact=True)

        with st.expander("📉 Compacte chart", expanded=True):
            if selected_result.get("entry_df") is not None:
        ...
        with st.expander("🔎 Andere coins", expanded=False):
            mobile_results = ranked_results or visible_results or scanner_results
            for result in mobile_results[:6]:
                coin = result.get("coin", "-")
                side_for_display = result.get("best_side") or result.get("primary_side") or result.get("context_preferred_side") or "-"
                result_price = result.get("live_price") or result.get("current_price")
                result_status = str(result.get("status") or "-")
                result_context = str(result.get("context_label_main") or result.get("trend_label") or "-")
                st.markdown(
                    f"""
                    <div class="bf-mobile-card" style="margin-bottom:8px;">
                        <div class="bf-mobile-title">{coin} • {side_for_display}</div>
                        <div class="bf-mobile-sub">Prijs: <strong>{fmt_price_eur(float(result_price)) if result_price is not None else '-'}</strong> • Context: {result_context} • Status: {_status_icon(result_status)} {result_status}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
                if st.button(f"Open {coin}", key=f"mobile_open_card_{coin}", use_container_width=True):
                    st.session_state.selected_coin = coin
                    st.rerun()

        with st.expander("Details / ladder", expanded=False):
